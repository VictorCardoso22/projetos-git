...
class AdminEmpresa {
    ...
    public function ExeCreate(array $Data) {
        ...
        if (in_array('', $this->Data)):
            ...
            if ($this->Data['empresa_capa']):
                $uplaod = new Upload;
                $uplaod->Image($this->Data['empresa_capa'], $this->Data['empresa_capa']);
            endif;

            if (isset($uplaod) && $uplaod->getResult()):
                $this->Data['empresa_capa'] = $uplaod->getResult();
                $this->Create();
            else:
                $this->Data['empresa_capa'] = null;
                $_SESSION['errCapa'] = "<b>ERRO AO ENVIAR CAPA:</b> Tipo de arquivo inválido, envie imagens JPG ou PNG!";
                $this->Create();
            endif;
        endif;
    }
    ...
    public function ExeUpdate($EmpresaId, array $Data) {
        ...
        if (in_array('', $this->Data)):
            ...
            if (is_array($this->Data['empresa_capa'])):
                $readCapa = new Read;
                $readCapa->ExeRead(self::Entity, "WHERE empresa_id = :post", "post={$this->Empresa}");
                $capa = '../uploads/' . $readCapa->getResult()[0]['empresa_id'];
                if (file_exists($capa) && !is_dir($capa)):
                    unlink($capa);
                endif;

                $uploadCapa = new Upload;
                $uploadCapa->Image($this->Data['empresa_capa'], $this->Data['empresa_title']);
            endif;

            if (isset($uploadCapa) && $uploadCapa->getResult()):
                $this->Data['empresa_capa'] = $uploadCapa->getResult();
                $this->Update();
            else:
    ...
    private function sendCapa() {
        if (!empty($this->Data['empresa_capa']['tmp_name'])):
            ...
            if ($w != '578' || $h != '288'):
                WSErro('Capa Inválida: A Capa da empresa deve ter exatamente 578x288px do tipo .JPG, .PNG ou .GIF!', WS_ALERT);
