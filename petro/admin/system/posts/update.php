...
<div class="content form_create">
    ...
    <article>
        ...
        if ($checkCreate && empty($cadastra)):
            WSErro("O post <b>{$post['post_title']}</b> foi cadastrada com sucesso no sistema!", WS_ACCEPT);
        endif;
        ?>
...
if ($Gallery->getResult()):
    foreach ($Gallery->getResult() as $gb):
        ...
        <img src='<?=HOME ?>/tim.php?src=<?=HOME ?>/uploads/<?=$gb['gallery_image'] ?>&w=146&h=100' alt='<?=$gbi?>' />
