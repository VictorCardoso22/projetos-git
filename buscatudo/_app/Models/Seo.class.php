        ...
        switch ($this->File):
            ...
            case 'categoria':
                ...
                if (!$ReadSeo->getResult()):
                    ...
                    $this->seoData = $ReadSeo->getResult()[0];
                    $this->Data = [$category_title . ' - ' . SITENAME, $category_content, HOME . "/categoria/{$category_name}", INCLUDE_PATH . '/imagens/site.png'];
            ...
            //SEO:: PESQUISA
            case 'pesquisa':
                $ReadSeo->ExeRead("app_empresas", "WHERE empresa_status = 1 AND (empresa_title LIKE '%' :link '%' OR empresa_ramo LIKE '%' :link '%' OR empresa_sobre LIKE '%' :link '%')", "link={$this->Link}");
                if (!$ReadSeo->getResult()):
                    $this->seoData = null;
                    $this->seoTags = null;
                else:
                    $this->seoData['count'] = $ReadSeo->getRowCount();
                    $this->Data = ["Pesquisa por: {$this->Link}" . ' - ' . SITENAME, "Sua pesquisa por {$this->Link} retornou {$this->seoData['count']} resultados!", HOME . "/pesquisa/{$this->Link}", INCLUDE_PATH . '/imagens/site.png'];
                endif;
                break;
            ...
            case 'empresas':
                ...
                $this->Data = ["Empresas {$this->Link} " . SITENAME, "Confira o guia completo de sua cidade, e encontra empresas de {$this->Link}.", HOME . '/empresas/' . $this->Link, INCLUDE_PATH . '/imagens/site.png'];
                break;
            ...
            case 'cadastra-empresa':
                $this->Data = ["Cadastre sua Empresa - " . SITENAME, "Entre em contado e divulgue sua empresa!", HOME . '/cadastra-empresa/' . $this->Link, INCLUDE_PATH . '/images/site.png'];
                break;

            case 'contato':
                $this->Data = ['Entre em contato conosco! - '. SITENAME, SITEDESC, HOME, INCLUDE_PATH . '/imagens/site.png'];
                break;
                //SEO::INFORMAÇÕES UTEIS
            case 'informacoes-uteis':
                $this->Data = ['SERVIÇO DE UTILIDADE E EMERGÊNCIA PÚBLICA. - '. SITENAME, SITEDESC, HOME, INCLUDE_PATH . '/imagens/site.png'];
                break;
             //SEO:: PORQUE ANUNCIAR
            case 'por-que-anunciar':
                $this->Data = ['Anuncie online e conquiste mais clientes. - ' . SITENAME, SITEDESC, HOME, INCLUDE_PATH . '/imagenns/site.png'];
                break;
             //SEO:: QUEM SOMOS
            case 'quem-somos':
                $this->Data = ['Quem Somos. - ' . SITENAME, SITEDESC, HOME, INCLUDE_PATH . '/imagens/site.png'];
                break;
            //SEO:: INDEX
            case 'index':
                $this->Data = [SITENAME . ' - Entrou, Achou, Saiu. É fácil!', SITEDESC, HOME, INCLUDE_PATH . '/imagens/site.png'];
                break;

            //SEO:: 404
            default :
                $this->Data = [SITENAME . ' - 404 Opa, aconteceu algo inesperado, nada foi encontrado!', SITEDESC, HOME . '/404', INCLUDE_PATH . '/imagens/site.png'];
